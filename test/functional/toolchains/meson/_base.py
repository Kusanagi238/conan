...
class TestMesonBase(unittest.TestCase):
    ...
    def _check_binary(self):
        ...
        if platform.system() == "Darwin":
            ...
            self.assertIn(f"main {arch_macro['gcc'][host_arch]} defined", self.t.out)
            # GNUC major version can vary across CI/toolchains; assert a numeric GNUC macro instead of a fixed value
            import re
            self.assertTrue(re.search(r"main __GNUC__\d+", self.t.out),
                            f"Expected GNUC major macro in output, got: {self.t.out}")
