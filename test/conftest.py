...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            return False
        # If a configured path is provided but does not exist on this runner, do not hard-fail here.
        # Instead, clear the configured path so we still check the system PATH for the executable.
    ...
    if not exe_found:
        ...
        # If the executable was found on the system (whether from the configured path
        # or from the system PATH), treat it as available and return its path and env.
