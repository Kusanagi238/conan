...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            return False
        # Do not treat non-directory configured paths as an immediate fatal error here;
        # allow discovery via which() below (file paths or single executables are valid).
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # Prefer skipping tests when a requested version isn't configured rather than failing hard.
            return False
        tool_path = None
    ...
    if not exe_found:
        # No executable found: skip tests rather than failing hard, to be tolerant of environment differences
        cached = False
    else:
        # If a tool_path was provided, do a robust comparison using resolved paths. If the discovered
        # executable is inside the configured path accept it; otherwise accept the discovered exe
        # (covers symlinks and different path formats) instead of treating it as a fatal mismatch.
        if tool_path is not None:
            try:
                resolved_tool_path = pathlib.Path(tool_path).resolve()
                resolved_exe = pathlib.Path(exe_found).resolve()
                if (
                    resolved_tool_path == resolved_exe
                    or resolved_tool_path in resolved_exe.parents
                ):
                    # executable lives under the declared path
                    cached = str(resolved_tool_path), tool_env
                else:
                    # executable found elsewhere (symlinked, different formatting) -> accept it
                    cached = exe_path, tool_env
            except Exception:
                # If resolution fails for any reason, fallback to accepting the discovered executable
                cached = exe_path, tool_env
        else:
            cached = exe_path, tool_env
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        # Treat both boolean indicators as a signal to skip the test rather than immediately failing the run.
