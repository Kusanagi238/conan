...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Configured path doesn't exist: do not fail immediately. Try to detect the executable on PATH.
            tool_path = None
    else:
        if (
            version is not None
        ):  # if a specific version is requested but not in the conf, try PATH first
            tool_path = None
        else:
            tool_path = None
    ...
    # Determine final result: return True only if the executable truly cannot be found;
    # otherwise return the discovered path (configured or detected) and any env vars.
    if not exe_found:
        # couldn't find executable on PATH (nor in configured path)
        return True

    if tool_path is not None:
        # executable found: ensure it lives under the configured path
        if tool_path not in exe_found:
            # found in a different location than configured -> treat as missing
            return True
        return tool_path, tool_env

    # No configured path: return the discovered exe location
    return exe_path, tool_env
