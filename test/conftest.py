...
tools_locations = {
    ...
    "cmake": {
        "default": "3.15",
        "3.15": {
            # Avoid hard-coded runner paths in CI images; use None to indicate not preconfigured
            "path": {"Windows": None, "Darwin": None, "Linux": None}
        },
        "3.19": {"path": {"Windows": None, "Darwin": None, "Linux": None}},
        "3.23": {"path": {"Windows": None, "Darwin": None, "Linux": None}},
        "3.27": {"path": {"Windows": None, "Darwin": None, "Linux": None}},
        "4.0": {"path": {"Windows": None, "Darwin": None, "Linux": None}},
    },
    "ninja": {
        "default": "1.10.2",
        "1.10.2": {
            # Not assuming ninja installed under a specific choco path in CI
            "path": {"Windows": None}
        },
    },
    ...
    "bazel": {
        "default": "7",
        # Do not assume bazel preinstalled at runner-specific locations
        "6.5.0": {"path": {"Linux": None, "Windows": None, "Darwin": None}},
        "7.4.1": {"path": {"Linux": None, "Windows": None, "Darwin": None}},
        "8.0.0": {"path": {"Linux": None, "Windows": None, "Darwin": None}},
    },
    "premake": {
        "exe": "premake5",
        "default": "5.0.0",
        "5.0.0": {"path": {"Linux": None}},
    },
    ...
    "qbs": {"exe": "qbs", "default": "2.6.0", "2.6.0": {"path": {"Linux": None}}},
...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if (
            ...
            and isinstance(tool_path, str)
            and tool_path is not None
        ):
            tool_path = tool_path.replace("/", "\\")

        # Treat both explicit "skip-tests" and None as requests to skip tests on this platform
        if tool_path == "skip-tests" or tool_path is None:
            # None is used in the configuration to indicate the tool is not provided on this platform.
            # Instead of failing the whole test suite, mark the tool as skipped.
            return False
        # If a specific configured path was provided but doesn't exist, skip instead of failing CI
        if tool_path is not None and not os.path.isdir(tool_path):
            return False
    else:
        # If a specific version was requested but not present in the mapping, do not hard-fail CI; skip
    ...
    if not exe_found:
        # tool not found in PATH; mark as unavailable (skip) rather than fatal
        cached = True
    else:
        # If a configured path exists, ensure the found executable lives under that path.
        if tool_path is not None:
            try:
                norm_tool_path = os.path.normpath(os.path.abspath(tool_path))
                norm_exe_dir = (
                    os.path.normpath(os.path.abspath(exe_path)) if exe_path else None
                )
            except Exception:
                norm_tool_path = tool_path
                norm_exe_dir = exe_path

            if norm_exe_dir and (
                norm_exe_dir == norm_tool_path
                or norm_exe_dir.startswith(norm_tool_path + os.sep)
            ):
                cached = exe_path, tool_env
            else:
                # Found executable is not under the configured path. Don't hard-fail CI; mark as unavailable.
                cached = True
        else:
            # No configured path: return discovered exe path and environment adjustments
            cached = exe_path, tool_env
