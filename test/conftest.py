...
def _get_individual_tool(name, version):
    ...
    # If version is None it means "any version" was requested by the test marker
    # and we should NOT force the configured default. Only when a version was
    # explicitly provided (possibly empty/False) use the configured default.
    ...
    if not exe_found:
        ...
        # Compare normalized absolute parent path of the found executable with the configured tool_path
        if exe_path is None:
            cached = True
        else:
            try:
                norm_tool = os.path.normcase(
                    os.path.normpath(os.path.abspath(tool_path))
                )
                norm_exe_parent = os.path.normcase(
                    os.path.normpath(os.path.abspath(exe_path))
                )
            except Exception:
                norm_tool = tool_path
                norm_exe_parent = exe_path
            if norm_tool != norm_exe_parent:
