...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Configured path does not exist: skip tests rather than failing them
            return False
    else:
    ...
    if not exe_found:
        # If no path was configured and the executable isn't found, this is a hard failure
        if tool_path is None:
            ...
            # Executable not found even after prepending configured path: skip tests
            if old_environ is not None:
                os.environ.clear()
                os.environ.update(old_environ)
            return False
