...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # If a configured path for a tool/version/platform does not exist in the CI
            # environment, treat it as unavailable but non-fatal (skip tests) instead
            # of a hard failure. This prevents test setup from aborting the whole
            # test run when optional external tools are not installed.
            return False
    else:
