...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # The configured path doesn't exist on this machine/CI. Do not treat this
            # as a hard failure here; unset tool_path and allow discovery from the
            # system PATH instead.
            tool_path = None
    else:
        # If the requested version isn't configured, allow falling back to system
        # discovery rather than failing immediately.
        tool_path = None
    ...
    if not exe_found:
        # Not found anywhere -> signal missing tool (upstream will decide fail/skip)
        cached = True
    else:
        exe_path = str(pathlib.Path(exe_found).parent)
        # Accept the discovered executable regardless of whether it lives under a
        # configured path. Using the found exe's parent path (exe_path) and any
        # configured environment ensures tests can use system-installed tools.
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            # Treat unavailable tools as skipped rather than failing the entire test
            # setup. This avoids hard setup failures for tools that may be optional
            # or available elsewhere in the environment.
