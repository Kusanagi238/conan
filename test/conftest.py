...
tools_locations = {
    ...
    "cmake": {
        "default": "3.15",
        "3.15": {
            # Avoid hardcoded CI runner paths for non-Windows platforms. If the
            # configured directory is not present in the environment, detection
            # should fall back to searching PATH.
            "path": {
...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured directory doesn't exist in this environment; don't fail
            # immediately, allow searching for the executable on PATH instead
            tool_path = None
    else:
    ...
    if not exe_found:
        ...
        if tool_path is None:
            # will fail the test, not exe found and no configured path -> fatal
            cached = True
    else:
        # executable found; if a configured tool_path was provided, prefer the
        # discovered executable (don't treat mismatch as fatal)
        if tool_path is not None:
            try:
                tp_norm = os.path.normpath(tool_path)
                ep_norm = os.path.normpath(exe_path) if exe_path else None
            except Exception:
                tp_norm = tool_path
                ep_norm = exe_path
            # If the exe is found in a different directory than the configured one,
            # accept the discovered executable and use its path/environment.
