...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # If a configured tool path does not exist on this machine (CI), do not treat
            # it as a hard failure. Instead, ignore the configured path and try to
            # discover the executable from the system PATH. This avoids failing tests
            # due to absolute paths that are only valid on specific images.
            tool_path = None
    else:
    ...
    if not exe_found:
        ...
        # Verify that the found executable is located under the configured tool_path.
        # Use resolved path ancestry instead of naive substring matching which can
        # falsely flag system-installed executables as mismatched.
        try:
            exe_parent = pathlib.Path(exe_found).parent.resolve()
            tool_path_resolved = pathlib.Path(tool_path).resolve()
            if (
                exe_parent != tool_path_resolved
                and tool_path_resolved not in exe_parent.parents
            ):
                # finds the exe in a path that is not the one set in the conf -> mark as mismatch
                cached = True
            else:
                cached = exe_path, tool_env
        except Exception:
            # Fallback to original substring check on unexpected errors
            if tool_path not in exe_found:
                cached = True
            else:
                cached = exe_path, tool_env
