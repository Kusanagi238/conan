...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Path configured but directory missing -> mark as unavailable (skip), not hard failure
            return False
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # Requested version not present -> mark as unavailable (skip)
            return False
        tool_path = None
    ...
    if not exe_found:
        # Executable not found -> mark as unavailable (skip)
        cached = False
        if tool_path is None:
            # tool not found and no configured path -> unavailable
            cached = False
    elif tool_path is not None and tool_path not in exe_found:
        # finds the exe in a path that is not the one set in the conf -> unavailable (skip)
        cached = False
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        # Treat missing or misconfigured tools as unavailable (skip) instead of hard failures
