...
tools_locations = {
    "clang": {"disabled": True},
    # Ensure GCC/g++ are mapped so CI uses an expected compiler version instead of an arbitrary system compiler
    "gcc": {
        "exe": "gcc",
        "default": "9",
        "9": {"path": {"Linux": "/usr/bin", "Darwin": "/usr/bin", "Windows": ""}},
    },
    "g++": {
        "exe": "g++",
        "default": "9",
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            ...
            # Treat unavailable external tools as a reason to skip the test in CI
            pytest.skip(
