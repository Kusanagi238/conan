...
def _get_individual_tool(name, version):
    ...
    if not exe_found:
        ...
        # Ensure the found executable actually lives under the configured tool_path.
        try:
            tool_path_p = pathlib.Path(tool_path).resolve()
            exe_parent_p = (
                pathlib.Path(exe_found).parent.resolve() if exe_found else None
            )
            # Accept if the executable's parent directory is exactly the tool_path
            # or is contained within the tool_path tree. Otherwise, treat as mismatch.
            if exe_parent_p is None or not (
                exe_parent_p == tool_path_p
                or str(exe_parent_p).startswith(str(tool_path_p) + os.sep)
            ):
                # found exe is not in the configured tool_path -> fail
                cached = True
        except Exception:
            # Fallback to a normalized string containment check
            if exe_found is None or os.path.normpath(tool_path) not in os.path.normpath(
                exe_found
            ):
                cached = True
