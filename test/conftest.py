...
tools_locations = {
    ...
    "pkg_config": {
        ...
        "default": "0.28",
        "0.28": {
            # Prefer searching on PATH or via package manager; specific CI runner paths removed
    ...
    "cmake": {
        "default": "3.15",
        # Do not hard-code runner-specific absolute paths. Allow detection via PATH or system packaging.
        "3.15": {},
        "3.19": {},
        "3.23": {},
        "3.27": {},
    ...
    "ninja": {"default": "1.10.2", "1.10.2": {"path": {"Windows": None}}},
    ...
    "bazel": {
        "default": "7",
        # Avoid hard-coded runner paths; allow system detection or PATH lookup
        "6.5.0": {},
        "7.4.1": {},
        "8.0.0": {},
    },
    "premake": {"exe": "premake5", "default": "5.0.0", "5.0.0": {}},
    ...
    "qbs": {"exe": "qbs", "default": "2.6.0", "2.6.0": {}},
    "emcc": {},
    "node": {},
...
def _get_individual_tool(name, version):
    ...
    if not exe_found:
        ...
        # Compare configured tool path to the directory containing the found executable
        # Use normalized absolute paths for robust comparison (handles separators/case on Windows)
        try:
            normalized_tool_path = os.path.normcase(os.path.abspath(tool_path))
            normalized_exe_dir = (
                os.path.normcase(os.path.abspath(exe_path)) if exe_path else None
            )
        except Exception:
            # Fallback to simple substring check if normalization fails for any reason
            normalized_tool_path = tool_path
            normalized_exe_dir = exe_path

        # If we couldn't determine the exe directory or it doesn't reside under the configured path -> treat as not matching
        if not normalized_exe_dir or not normalized_exe_dir.startswith(
