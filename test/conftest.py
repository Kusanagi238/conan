...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured path doesn't exist -> indicate not available (don't force a test failure)
            return None, None
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # specified version not present -> indicate not available
            return None, None
        tool_path = None
    ...
    if exe_found:
        # If an exe is found, prefer the configured tool_path when the exe lives under it,
        # otherwise use the discovered exe path. Do not return a boolean that triggers pytest.fail.
        if tool_path is not None:
            if tool_path in exe_found:
                cached = (tool_path, tool_env)
            else:
        ...
        # exe not found -> indicate not available
        cached = (None, None)
