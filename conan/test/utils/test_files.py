...
def temp_folder(path_with_spaces=True, create_dir=True):
    ...
    if (
        not path_with_spaces
        or platform.system() == "FreeBSD"
...
def uncompress_packaged_files(paths, pref):
    ...
    # pref.revision should be the recipe revision (rev). The previous code
    # mistakenly assigned the package revision (prev) to pref.revision.
    pref.revision = rev
...
def tgz_with_contents(files, output_path=None):
    ...
    with open(file_path, "wb") as tgz_handle:
        # Open a gzip writer without timestamps, then open a tarfile on top of it
        # so we add tar entries correctly into the gzip stream.
        with gzopen_without_timestamps(file_path, fileobj=tgz_handle) as gz:
            with tarfile.open(fileobj=gz, mode="w:") as tgz:
