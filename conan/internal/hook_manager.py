...
class HookManager:
    ...
    def execute(self, method_name, conanfile):
        ...
        for name, method in hooks:
            ...
            try:
                conanfile.display_name = "%s: [HOOK - %s] %s()" % (
                ...
                # Preserve original exception context so the original exception type and traceback
                # are visible to aid debugging
                raise ConanException(
                    "[HOOK - %s] %s(): %s" % (name, method_name, str(e))
                ) from e
            finally:
